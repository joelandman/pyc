...
public:
    ...
    std::string run() {
        ...
        fn_locs_.assign(m_.functions.size(), {});
        for (std::size_t i = m_.functions.size(); i-- > 0;) emit_function(i);
        emit_pyconsts();
        emit_constants();
        emit_declarations();
        return o_.str();
...
private:
    ...
    void emit_instr(const ir::Function& f, const ir::Instr& in, bool is_main) {
        ...
        switch (in.op) {
            ...
            case Op::MakeFunction: {
                need("declare ptr @pyc_rt_make_function(ptr, ptr, i32, i32, i32, i32, ptr, ptr, ptr, i32, i32, ptr, i32, i32, ptr, i32)");
    ...
    std::string loc_global(std::size_t idx) {
        if (idx >= fn_locs_.size() || fn_locs_[idx].empty()) return "null";
        return "@.locs" + std::to_string(idx);
    }
    ...
    void emit_constants() {
        ...
        for (std::size_t i = 0; i < fn_locs_.size(); ++i) {
            const auto& v = fn_locs_[i];
            if (v.empty()) continue;
            o_ << "@.locs" << i << " = private unnamed_addr constant ["
               << v.size() << " x i32] [";
