...
public:
    ...
    std::string run() {
        ...
        for (const ir::Function& f : m_.functions) emit_function(f);
        emit_constants();
        emit_declarations();
        return o_.str();
    }
...
private:
    ...
    void emit_function(const ir::Function& f) {
        ...
        o_ << (is_main ? "define i32 " : "define ptr ") << fname(f)
           << "(ptr %locals) {\n";
    ...
    void emit_block(const ir::Function& f, const ir::Block& b,
        ...
        if (b.instrs.empty() || !is_term(b.instrs.back().op))
            o_ << "  unreachable\n";
    }
    ...
    const ir::Function* find(const std::string& name) const {
        for (const ir::Function& f : m_.functions) if (f.name == name) return &f;
        return nullptr;
    }
