...
Open: none in this directory. Fixed, kept as regression cover: **B**
(comprehension cells), **C** (nested-class closures), **D** (C-stack
alloca). **A** (frames, C1) and **E** (GIL, C2) moved to `language/`.
...
## E: a compiled loop never offers the GIL — FIXED (C2)

`thread_starvation.py` moved to `language/`. `pyc_rt_periodic` calls
`_Py_HandlePending`, which detaches the GIL only when
`_PY_GIL_DROP_REQUEST_BIT` is set. Unconditional SaveThread/RestoreThread
fixed starvation and deadlocked `test_logging`; HandlePending does not:
`test_syslog` 11/11 in 0.18s, `test_logging` finishes in 25s (no deadlock).

Do not revert the alloca hoist (`6db99ec`) to buy `test_syslog` by luck:
that reintroduces C-stack exhaustion on any call in a loop.
