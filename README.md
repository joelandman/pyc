...
the normal compiler path. **306/306 tests passing** (runner reports 306/306, file_case_failures=0; every case compared against CPython output; see `tests/runner.py`).
...
- **Standard library stubs**: `os` (path helpers, getcwd, listdir, makedirs, environ), `pathlib` (`Path`), `subprocess` (call, check_output), `sys`, `cmath`, `math` (~25 functions wrapping libm), `json` (dumps/loads), `random` (CPython-exact MT19937), `itertools`/`collections` (eager-list subset), `datetime` (`date`/`datetime`/`timedelta` — see FEATURES.md)
