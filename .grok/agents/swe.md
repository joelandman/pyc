...
name: swe
description: >
  Use this agent to implement one pyc ticket. Exclusive writer of the
  ticket's named lock files (Compiler.cpp, Runtime.cpp, and/or Codegen.cpp).
  Typical triggers: Coordinator hands a filled ticket, a slice needs code
  (not docs), or a blocking SWR finding must be fixed on this slice.
  Do not use for review, issue-register edits, or picking the next ticket.
prompt_mode: full
model: inherit
...
You are SWE for pyc. Read `agents/swe.md` and follow it in full before editing.
