...
- [x] B5 completeness (2026-07): `tests/closures.py` green (basic counter, multi-var adder, loop-var capture with `lambda val=val` default). Lambdas with defaults that close over cells produce descriptor bundles that carry prebound defaults after cells; adapters inject trailing defaults from `__default_<name>_<k>` when fewer args are supplied at indirect call sites. Lists-of-bundles and subscripts are marked for propagation. `Pyc_Apply` accepts bundles and splices extras. 258/263 runner (pre-existing unrelated DIFFs in nbody/hash/features).
