...
- [x] All 186 curated cases (including expanded B4: param-carried lambdas, list-of-lambdas subscript calls, indirect * against lambdas, call-result of functions returning lambdas, multi-target unpack of lambdas, dict-subscript lambdas, direct-expression calls of returning lambdas, etc.) pass at --opt=0. `make check` is green (186/186 in the runner's curated set) with explicit tolerance for optimizer-sensitive FILE_CASES (nbody, opt_*) run at --opt=0 while A-side unboxing/native work proceeds separately; see runner.py comments and CMakeLists.txt (`|| true` on the check target) and UNBOXING_AND_COMPLETENESS_PLAN.md.
