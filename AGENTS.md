...
- `CASES` (lines 7–3567): 595 inline source/expected pairs. Compiled at
...
- `FILE_CASES` (lines 3568–3609): 29 real `.py` programs in `tests/`, each compiled
...
`./build/pyc`, etc. (runner.py:3615–3633). The 5s per-command `timeout` in
`run()` (runner.py:3611) bites slow programs.
